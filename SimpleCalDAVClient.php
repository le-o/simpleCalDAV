...
require_once(dirname(__FILE__) . '/CalDAVClient.php');
require_once(dirname(__FILE__) . '/CalDAVException.php');
require_once(dirname(__FILE__) . '/CalDAVFilter.php');
require_once(dirname(__FILE__) . '/CalDAVObject.php');
...
class SimpleCalDAVClient {
	 ...
	 * For debugging purposes, just sorround everything with try { ... } catch (Exception $e) { echo $e->__toString(); exit(-1); }
	 */
	function create($cal, $force = false)
	{
		...
		if (false == $force && $this->client->GetHttpResultCode() != '201' )
		{
	...
	function delete ( $href, $etag )
	{
		...
		// Do the deletion
		$response = $this->client->DoDELETERequest($href, $etag);
	...
	function directDelete ( $href, $etag )
	{
		try {
			// Connection and calendar set?
			if(!isset($this->client)) throw new Exception('No connection. Try connect().');
	...
	function getCustomReport ( $filterXML )
	{
		try {
			...
			// GET-request successfull?
			if ( $this->client->GetHttpResultCode() != '207' && $this->client->GetHttpResultCode() != '412' )
			{
				throw new CalDAVException('Recieved unknown HTTP status '.$this->client->GetHttpResultCode().' -- '.$filterXML, $this->client);
			}

			// Reformat
			$report = array();
			foreach($results as $event) {
				$report[] = new CalDAVObject($this->url.$event['href'], $event['data'], $event['etag']);
			}

			return $report;
		} catch (Exception $e) {
			echo $e->__toString(); exit(-1);
		}
	}
	...
	function GetEntryByUid($uid) {
		return $this->client->GetEntryByUid($uid);
	}
